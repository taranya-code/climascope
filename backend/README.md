...
54 tests: pure-function unit tests for each domain module (`tests/test_solar.py`,
`test_wind.py`, `test_climate_risk.py`, `test_adaptation.py`) against hand-computed
expected values, a mocked-HTTP test for the NASA POWER client
(`test_nasa_power_client.py`), and API integration tests against an in-memory
...
- `app/domain/` — pure calculation functions (solar, wind, climate risk, adaptation
  recommendations). No I/O; this is where the interesting logic lives.
...
- `app/routers/` — `POST /assess` and the `/sites` CRUD endpoints.
